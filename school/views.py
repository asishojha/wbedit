...
def login_view(request):
    ...
    if form.is_valid():
        ...
        (
            ...
            created_change_password,
        ) = Permission.objects.get_or_create(
            codename="can_change_password",
            name="Can change the password",
            content_type=ct,
        )
...
def submit_final_data(request):
    ...
    if pending_students.count():
        messages.warning(
            request,
            "Please make sure that all the students are marked as either SELECTED or NOT SELECTED",
        )
        return redirect("school:student_list")
    ...
    if profile.complete:
        messages.success(
            request,
            "Final Data is already submitted for your students. Please download the report.",
        )
        return redirect("school:student_list")
    ...
    if request.method == "POST":
        ...
        if agree:
            ...
            messages.success(
                request,
                "Congratulations! All your data has been finally submitted. Your can now download the report generated.",
            )
            return redirect("school:student_list")
