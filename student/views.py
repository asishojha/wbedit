...
def student(request, serial):
    ...
    if (student.f_name == "" or student.f_name is None) and (
        student.g_indicator == "" or student.g_indicator is None
    ):
        if (student.m_name == "" or not student.m_name) and (
            student.f_name == "" or not student.f_name
        ):
    ...
    context = {"student": student, "blank_warning": blank_warning}
...
def select_student(request, serial):
    ...
    if (not student.f_name and not student.m_name and student.g_name) or (
        student.f_name == "" and student.m_name == "" and student.g_name
    ):
        extra_warning = "Warning! Father's Name and Mother's Name are blank. "

    if (not student.f_name and not student.m_name and not student.g_name) or (
        student.f_name == "" and student.m_name == "" and student.g_name == ""
    ):
        messages.error(
            request,
            f"Student with \"Blank Father's Name, Mother's Name or Guardian's Name\",  {student.name} can not be selected.",
        )
        ...
        messages.error(
            request,
            f'Student with "Blank Date of Birth",  {student.name} can not be selected.',
        )
        return redirect(student.get_absolute_url())

    elif datetime.strptime(dob, "%d%m%y") and datetime.strptime(
        dob, "%d%m%y"
    ) > datetime.strptime("311010", "%d%m%y"):
        messages.error(
    ...
    try:
        next_student = Student.objects.filter(
            school=request.user, serial__gt=serial
        ).first()
        messages.success(
            request,
            f"{extra_warning}Student {student.name} has been marked as Selected.",
        )
...
def not_select_student(request, serial):
    ...
    try:
        ...
        messages.warning(
            request,
            f"{extra_warning}Student {student.name} has been marked as NOT Selected",
        )
        return redirect(next_student.get_absolute_url())
    except Exception:
        return redirect("school:student_list")
...
def edit_student(request, serial):
    ...
    student = Student.objects.filter(school=request.user, serial=serial)[0]
    student_dict = model_to_dict(
        student, fields=[field.name for field in student._meta.fields]
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            original_profile_picture = Student.objects.get(
                school=request.user, serial=serial
            ).profile_picture
            ...
            if edited == "1":
                ...
                s_document, s_document_created = SupportDocument.objects.get_or_create(
                    student=student
                )
            ...
            messages.success(
                request,
                f"Information for student {student.name} has been updated. Please SELECT / NOT SELECT the student.",
            )
            return redirect(student.get_absolute_url())
